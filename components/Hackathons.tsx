...
const HACKATHONS = [
  {
    ...
    project: "QuietHelp",
    tagline: "Silence should not mean you cannot be heard.",
...
// ─── Hackathon Card ──────────────────────
function HackathonCard({ h, delay }: { h: typeof HACKATHONS[0]; delay: number }) {
        ...
        <div className="relative h-48 w-full overflow-hidden">
          ...
          {h.awards.length > 0 && (
            <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
              {h.awards.map((a) => (
                <span key={a} className="text-xs bg-rose-800/90 text-white px-2.5 py-1 rounded-full font-medium backdrop-blur-sm">
                  {a}
                </span>
              ))}
            </div>
